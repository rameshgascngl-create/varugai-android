...
grep -q 'enabled = holidayDay != null' "$SETUP" || fail "holiday action must enable after an in-semester date is selected"
! grep -q 'holidayDay != null && holidayName.isNotBlank()' "$SETUP" || fail "holiday action still incorrectly requires a name"
grep -q 'onMarkHoliday' "$SETUP" || fail "dedicated holiday action wiring missing"
grep -q 'suspend fun markHoliday' "$REPO" || fail "repository holiday action missing"
grep -q 'Clear that day.*attendance first' "$REPO" || fail "holiday conversion must protect existing attendance"
...
grep -q 'countOutsideDateRange' "$REPO" || fail "non-destructive semester-range attendance guard missing"
grep -q 'countHoursAbove' "$REPO" || fail "non-destructive daily-hour attendance guard missing"
grep -q 'Calendar copy is blocked because the target register already has attendance' "$REPO" || fail "calendar-copy attendance protection missing"
pass "setup/calendar hardening and attendance preservation"
